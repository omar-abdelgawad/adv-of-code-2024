...
fn is_safe(report: &Report) -> bool {
    let mut is_safe_all_tries = false;
    is_safe_all_tries |= is_safe_helper(report);
    for i in 0..report.len() {
        if is_safe_all_tries {
            return true;
        }
        let skipped: Vec<i64> = report
            .iter()
            .enumerate()
            .filter(|(j, _)| *j != i)
            .map(|(_, val)| *val)
            .collect();
        is_safe_all_tries |= is_safe_helper(&skipped);
    }
    is_safe_all_tries
}

fn is_safe_helper(report: &Report) -> bool {
    if report.len() <= 1 {
        return true;
    }
...
39 40 43 44 50";
    ...
    fn test_safe() {
        ...
        for (i, test) in tests.iter().enumerate() {
            assert!(is_safe(test), "test no.: {} failed", i + 1);
        }
    }

    #[test]
    fn test_not_safe() {
        let tests: Vec<Vec<i64>> = vec![vec![1, 2, 7, 8, 9], vec![9, 7, 6, 2, 1]];
        for (i, test) in tests.iter().enumerate() {
            assert!(!is_safe(test), "test no.: {} failed", i + 1);
        }
    }
