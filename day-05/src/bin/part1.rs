...
fn main() {
    ...
    dbg!(filename);
    let (edges, updates) = get_part_1_input(filename);
    let ans = part_1(&edges, &updates);
    dbg!(ans);
}
fn part_1(edges: &[(i64, i64)], updates: &[Vec<i64>]) -> i64 {
    let orderings: HashSet<(i64, i64)> = edges.iter().copied().collect();
    let compare_func = |x: &i64, y: &i64| orderings.contains(&(*x, *y));
    let mut midlle_sum = 0;
    for update in updates {
        if update.is_sorted_by(compare_func) {
...
";
        ...
        assert_eq!(part_1(&edges, &updates), 143);
    }
}
