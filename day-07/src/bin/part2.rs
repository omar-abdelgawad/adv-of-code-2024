...
fn part_2(equations: Equations) -> u64 {
    let mut ans = 0;
...
fn is_equation_valid_helper(test_value: u64, nums: &Vec<u64>, ind: usize, accum: u64) -> bool {
    ...
    let new_accum_concat: Result<u64, _> = format!("{}{}", accum, nums[ind]).parse();
    ...
    if let Ok(accum_concat) = new_accum_concat {
        return is_equation_valid_helper(test_value, nums, ind + 1, accum_concat);
    }
    false
}
